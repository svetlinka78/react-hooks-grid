...
export const useFiltering = (initFilters, customPredicates) => updateGrid => {
    ...
    setFilterFn(() =>
      composeFilters(
        ...Object.values(newFilters).map(f => {
          ...
          if (f.type === 'greaterDate') return filterGreaterDate(f);
          if (f.type === 'lesserDate') return filterLesserDate(f);
  ...
  const filterGreaterDate = filter => row => new Date(row[filter.column]) >  new Date(filter.value);
  const filterLesserDate = filter => row => new Date(row[filter.column]) >  new Date(filter.value);
